...
### bubbletea-tui
- Path: /home/marcd/workspace/daimon/.agents/skills/bubbletea-tui/SKILL.md
- Trigger: Trigger: Bubbletea, Bubble Tea, TUI, Lipgloss, terminal UI, Charm, daimon power-user UI. Guides building daimon's embedded TUI with Charm v1 (bubbletea v1.3.10 / lipgloss v1.1.0 / bubbles v1.0.0).
- Rules:
  - **Charm v1 ONLY.** Pinned: `bubbletea v1.3.10`, `lipgloss v1.1.0`, `bubbles v1.0.0`, `x/ansi v0.11.6`. NEVER import `charm.land/*` paths. NEVER reference v2 APIs (`uv.Screen`, `ScreenBuffer`, `Draw(scr, area)`).
  - **SINGLE ROOT MODEL.** One struct owns `tea.Model`. Sub-components are plain structs with imperative methods (`Render(w int) string`, `SetMessages(...)`, `Focus()`). NEVER nest Elm sub-models with their own `Update(tea.Msg)`.
  - **Centralized styles.** All Lipgloss styles live in one semantic struct (like `dashStyles`). NEVER hardcode colors inline. Thread the struct to sub-components.
  - **ANSI width math.** Use `github.com/charmbracelet/x/ansi` (`StringWidth`, `Truncate`, `Strip`) for every truncation/alignment. NEVER slice ANSI strings at byte level.
  - **Cmd discipline.** IO/expensive work → return a `tea.Cmd`. No state mutation inside a command closure. Multiple cmds → `tea.Batch(...)`.
  - **Layout sizing.** Compute rectangles from `WindowSizeMsg`. Always subtract border+padding widths before passing to children (off-by-N is the #1 truncation bug).
  - **Overlays last.** Draw all dialog overlays after the main layout; use an overlay manager with push/pop keyed by dialog ID.
  - **Embedded dispatch.** The TUI calls daimon's internal command registry directly (same process). NEVER make REST calls from Update.
  - **Run entry point pattern.** Expose a `RunXxx(cfg) error` function, call `tea.NewProgram(m, tea.WithAltScreen())`, require TTY (see `RunMCPManage`).
  - Situation: Decision
...
### golang-patterns
- Path: /home/marcd/workspace/daimon/.agents/skills/golang-patterns/SKILL.md
...
### golang-pro
- Path: /home/marcd/workspace/daimon/.agents/skills/golang-pro/SKILL.md
...
### kanon-create-issue
- Path: /home/marcd/.claude/skills/kanon-create-issue/SKILL.md
- Trigger: Natural Language Issue Creation — parse user descriptions of bugs, features, and tasks into well-structured Kanon issues without manual field-filling
- Rules:
  - No compact rules declared; delegators should load the full skill file before direct work, or pass an explicit fallback path only when Project Standards cannot be injected.

### kanon-cycle
- Path: /home/marcd/.claude/skills/kanon-cycle/SKILL.md
- Trigger: Cycle planning, scope management, and close discipline — create cycles, attach/detach issues with audit trail, read burnup/risks, and choose the right close disposition.
- Rules:
  - No compact rules declared; delegators should load the full skill file before direct work, or pass an explicit fallback path only when Project Standards cannot be injected.
...
### kanon-mcp
- Path: /home/marcd/.claude/skills/kanon-mcp/SKILL.md
- Trigger: Human-facing project board integration — clean cards, meaningful titles, progressive enrichment from SDD and general work
- Rules:
  - No compact rules declared; delegators should load the full skill file before direct work, or pass an explicit fallback path only when Project Standards cannot be injected.
...
- Path: /home/marcd/.claude/skills/kanon-orchestrator-hooks/SKILL.md
- Trigger: Kanon-specific hooks for the SDD orchestrator — ROADMAP injection into sub-agent prompts and post-phase deferred_items processing
- Rules:
  - No compact rules declared; delegators should load the full skill file before direct work, or pass an explicit fallback path only when Project Standards cannot be injected.

### kanon-roadmap
- Path: /home/marcd/.claude/skills/kanon-roadmap/SKILL.md
- Trigger: Proactive roadmap capture — recognize future work during conversations and SDD workflows, create and enrich roadmap items automatically
- Rules:
  - No compact rules declared; delegators should load the full skill file before direct work, or pass an explicit fallback path only when Project Standards cannot be injected.
