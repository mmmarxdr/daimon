...
2. **Media persistence in SQLiteStore**: store binary blobs inline in the `conversations` JSON (easy, bad for large files), write to `~/.microagent/data/media/<hash>` and store a path reference (good, requires cleanup), or use a content-addressable store (CAS) keyed by SHA256? Voice notes can be several MB; photos up to 10 MB.
