...
Defines the `minimax` provider type: OpenAI-compatible transport to MiniMax API, plus stateful `<think>...</think>` tag stripping/routing in both the sync and streaming chat paths. In-think content is routed to the reasoning channel (`ReasoningDelta`); only out-of-think content appears in `ChatResponse.Content`.

## Requirements

### Requirement: MiniMax-MM-1 — Provider construction and identity
...
### Requirement: MiniMax-MM-2 — Sync path: think-tag stripping
...
### Requirement: MiniMax-MM-3 — Streaming path: split-safe tag detection
...
### Requirement: MiniMax-MM-4 — Streaming: unclosed or malformed think tag
...
### Requirement: MiniMax-MM-5 — Tag-detection buffer is bounded
...
### Requirement: MiniMax-MM-6 — Tool calls are unaffected by stripping
