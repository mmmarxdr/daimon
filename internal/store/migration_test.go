...
func TestMigration_FreshDBHasSchemaVersion(t *testing.T) {
	...
	// Fresh DB goes through all migrations → latest version is 7.
	if version != 7 {
		t.Errorf("expected schema_version=7 on fresh DB, got %d", version)
	}
}
...
func TestMigration_RerunIsNoOp(t *testing.T) {
	...
	if version != 7 {
		t.Errorf("expected schema_version=7 after re-run, got %d", version)
	}
	s.Close()
}
...
`
	...
	if version != 7 {
		t.Errorf("expected schema_version=7 after v1→v2→v3→v4→v5→v6→v7, got %d", version)
	}
...
func TestMigration_V3_SchemaVersionIs3(t *testing.T) {
	...
	if version != 7 {
		t.Errorf("expected schema_version=7, got %d", version)
	}
}
