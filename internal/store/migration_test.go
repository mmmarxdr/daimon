...
func TestMigration_FreshDBHasSchemaVersion(t *testing.T) {
	...
	if version != 17 {
		t.Errorf("expected schema_version=17 on fresh DB, got %d", version)
	}
}
...
`
	...
	if version != 17 {
		t.Errorf("expected schema_version=17 after all migrations, got %d", version)
	}
...
func TestMigration_V3_SchemaVersionIs3(t *testing.T) {
	...
	if version != 17 {
		t.Errorf("expected schema_version=17, got %d", version)
	}
}
