...
func TestMigration_FreshDBHasSchemaVersion(t *testing.T) {
	...
	// Fresh DB goes through all migrations → latest version is 10.
	if version != 10 {
		t.Errorf("expected schema_version=10 on fresh DB, got %d", version)
	}
}
...
func TestMigration_RerunIsNoOp(t *testing.T) {
	...
	if version != 10 {
		t.Errorf("expected schema_version=10 after re-run, got %d", version)
	}
	s.Close()
}
...
`
	...
	if version != 10 {
		t.Errorf("expected schema_version=10 after v1→v2→v3→v4→v5→v6→v7→v8→v9→v10, got %d", version)
	}
...
// migrations complete (now 5, as v5 media_blobs migration was added).
func TestMigration_V3_SchemaVersionIs3(t *testing.T) {
	...
	if version != 10 {
		t.Errorf("expected schema_version=10, got %d", version)
	}
}
