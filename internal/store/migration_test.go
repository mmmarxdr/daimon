...
func TestMigration_FreshDBHasSchemaVersion(t *testing.T) {
	...
	if version != 13 {
		t.Errorf("expected schema_version=13 on fresh DB, got %d", version)
	}
}
...
func TestMigration_RerunIsNoOp(t *testing.T) {
	...
	if version != 13 {
		t.Errorf("expected schema_version=13 after re-run, got %d", version)
	}
	s.Close()
}
...
`
	...
	if version != 13 {
		t.Errorf("expected schema_version=13 after all migrations, got %d", version)
	}
...
func TestMigration_V3_SchemaVersionIs3(t *testing.T) {
	...
	if version != 13 {
		t.Errorf("expected schema_version=13, got %d", version)
	}
}
